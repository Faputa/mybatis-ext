...
public abstract class BaseParser {
    ...
    protected Symbol choice(Symbol... symbols) {
        ...
        return choice.setLeftSymbols(symbols).setMatch((state, tokenizer, continuation) -> {
            int cursor = tokenizer.getCursor();
            for (Symbol symbol : symbols) {
                if (symbol.match(new State(state, state.getScope()), tokenizer, continuation)) {
                    return true;
                }
    ...
    protected Symbol join(Symbol... symbols) {
        ...
        return join.setLeftSymbols(head).setMatch((state, tokenizer, continuation) -> {
            return head.match(new State(state, state.getScope()), tokenizer, tail == null ? continuation : s2 -> {
                return tail.match(new State(s2, state.getScope()), tokenizer, continuation);
            });
        });
    }
    ...
    protected Symbol optional(Symbol symbol) {
        ...
        return optional.setLeftSymbols(symbol).setMatch((state, tokenizer, continuation) -> {
            int cursor = tokenizer.getCursor();
            if (symbol.match(new State(state, state.getScope()), tokenizer, continuation)) {
                return true;
            }
    ...
    private boolean matchStar(Symbol symbol, State state, Tokenizer tokenizer, Continuation continuation) {
        int cursor = tokenizer.getCursor();
        if (symbol.match(new State(state, state.getScope()), tokenizer, s2 -> {
            return matchStar(symbol, s2, tokenizer, continuation);
        })) {
            return true;
        }
        tokenizer.setCursor(cursor);
    ...
    protected Symbol star(Symbol symbol) {
        ...
        return star.setLeftSymbols(symbol).setMatch((state, tokenizer, continuation) -> {
            return matchStar(symbol, state, tokenizer, continuation);
        });
    }

    protected Symbol plus(Symbol symbol) {
        Symbol plus = new Symbol("plus(" + symbol + ")");
        return plus.setLeftSymbols(symbol).setMatch((state, tokenizer, continuation) -> {
            return symbol.match(new State(state, state.getScope()), tokenizer, s2 -> {
                return matchStar(symbol, s2, tokenizer, continuation);
            });
        });
    }
    ...
    protected Symbol count(Symbol symbol, int num) {
        ...
        return count.setLeftSymbols(symbol).setMatch((state, tokenizer, continuation) -> {
            return symbol.match(new State(state, state.getScope()), tokenizer, countDec == null ? continuation : s2 -> {
                return countDec.match(new State(s2, state.getScope()), tokenizer, continuation);
            });
        });
    }
    ...
    protected Symbol assign(String name, Symbol symbol) {
        ...
        return assign.setLeftSymbols(symbol).setMatch((state, tokenizer, continuation) -> {
    ...
    protected Symbol action(Predicate<State> predicate) {
        ...
        return action.setMatch((state, tokenizer, continuation) -> {
